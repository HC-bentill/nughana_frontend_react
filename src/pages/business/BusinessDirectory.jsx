...
import {
  ...
  GetBusinessDirectoriesByCategoryById,
...
function BusinessDirectory() {
  ...
  const GetBusinessDirectories = useQuery({
    ...
    queryFn: () =>
      GetBusinessDirectoriesByCategoryById(activeTab).then((res) => res?.data),
  });
  ...
  return (
    <>
      ...
      {GetBusinessDirectories?.isLoading ||
      GetBusinessDirectories?.isFetching ? (
        ...
        <div className="md:flex justify-between md:ml-3 md:px-0 gap-5">
          <div className="flex-1">
            ...
            <div className="flex gap-2 my-5">
              <div className="border-[#E5E5E7] bg-[#ffffff] border rounded-[30px] flex items-center gap-2 md:w-60 w-[90%] md:mx-0 mx-auto md:mb-0 mb-7">
                ...
                <input
                  ...
                  type="text"
                  placeholder="Search in Business Directory"
                />
              </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {GetBusinessDirectories?.data?.length > 0 &&
                GetBusinessDirectories?.data?.map((am, i) => (
                  <BusinessCard
                    otherClassNames={""}
                    key={i}
                    img={am?.acf?.business_image}
                    footerImgs={[call, whatsapp, mail]}
                    footerIconsWith={"w-[24px] mr-2"}
                    header={am?.business_name}
                    desc={am.desc}
                    eventfooterDesc={"View in google maps"}
                    name={am?.acf?.business_name}
                    address={am?.acf?.address}
                    id={am?.id}
                    link={am?.acf?.google_map_link_to_business}
                  />
                ))}
            </div>
          </div>
