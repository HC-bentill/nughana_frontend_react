...
import { GetMemberDetails } from '../../api/connections.service';
...
const NavigationBar = () => {
   ...
   return (
      <>
         <div className="fixed top-0 z-50 w-full">
            <div className="border-b-[0.5px] border-[#bfbfbf64] bg-[white]">
               <div className="flex items-center md:gap-0 gap-4 justify-between py-3 mx-auto md:max-w-[90%] md:px-0 px-7">
                  <div className="">
                     <img onClick={() => navigate('/')} src={NuGhanaLogo} alt="img" className="w-[40px] h-[40px] md:ml-[18px] shadow-sm" />
                  </div>
                  <div className="border-[#E5E5E7] border rounded-[30px] flex items-center gap-2 w-52 md:w-72">
                     ...
                     <input className="text-[12px] font-light border-0 outline-none" type="text" placeholder="Search" />
                  </div>
                  <div className="flex items-center md:gap-5">
                     ...
                     <div className="flex items-center" onClick={() => handleProfileNavToggle()}>
                        <div
                           ...
                           style={{
                              backgroundImage: `url(${getMemberDetails?.data?.avatar_urls?.full})`,
                           }}
                        >
                           <div className="bg-transparent border-[2px] border-white rounded-full w-8 h-8 cursor-pointer"></div>
                        </div>
                        <img src={Dropdown} alt="img" className="w-[12px] md:block hidden  h-[12px] object-contain shadow-sm m-2" />
                     </div>
                  </div>
               </div>
            </div>
         ...
         <div className=" bg-[white] w-full md:hidden block fixed top-[63px] z-20">
            <div className="flex items-center justify-between px-8 py-2 pt-3">
               <div className="flex flex-col items-center">
                  <img onClick={() => navigate('/')} src={HomeMobile} alt="img" className="object-contain cursor-pointer" />
                  <p className="text-[8px] pt-1">Home</p>
               </div>

               {sidemenuIcons?.slice(0, 3).map((item, idx) => (
                  <div key={idx} className="flex flex-col items-center">
                     <img onClick={() => navigate(item.route)} src={item.mobileIcon} alt="img" className="object-contain cursor-pointer" />
                     <p className="text-[8px] pt-1">{item.label}</p>
                  </div>
               ))}
               <div className="flex flex-col items-center mt-2 cursor-pointer" onClick={() => handleMenuNavToggle()}>
                  <img onClick={() => handleProfileNavToggle} src={ThreeDotsMenu} alt="img" className="object-contain cursor-pointer" />
         ...
         <div id="mySidenav" ref={sidenavRef} className={`h-full fixed top-0 right-0 bg-[white] overflow-x-hidden z-30 shadow ${handleNav ? 'w-[75%] md:w-[50%] lg:w-[25%]' : 'w-0'}`}>
            <div className="text-black items-center flex justify-end py-3 px-5 mt-[60px]">
               <div>
                  <AiOutlineClose size={'1em'} onClick={handleProfileNavToggle} className="mt-2 cursor-pointer" color="black" />
               </div>
            </div>
            <div className="mt-0 text-[14px] h-full w-[86%] mx-auto">
               <div className="flex gap-3 items-center bg-[#F8F9FA] border border-gray-100 rounded-[20px] p-[25px]">
                  ...
                  <div>
                     ...
                     <p className="text-[13px] text-[#AF7E00] cursor-pointer" onClick={handleNavigateToProfile}>
                        View my profile
                     </p>
                  </div>
               </div>
         ...
         {/* responsive menu nav */}
         <div className={`h-full fixed top-0 right-0 bg-[white] overflow-x-hidden z-30 shadow ${handleMenuNav ? 'w-[100%]' : 'w-0'}`}>
            <div className="mt-[96px] text-[14px] w-[86%] mx-auto">
               <div className="flex items-center justify-between">
                  <img onClick={handleMenuNavToggle} className="cursor-pointer" src={RoundedLeftChev} alt="" />
               ...
               <div className="pt-5 font-light">
                  <div className="border-b-[1px] border-t-[1px] border-gray-100 py-1">
                     <MobileMenuItem icon={HomeMobile} label={'Home'} href={'/'} onSelect={handleMenuNavToggle} />
                  </div>
                  {sidemenuIcons?.map((item, idx) => (
                     <div key={idx} className="border-b-[1px] border-gray-100 py-1">
                        <MobileMenuItem icon={item.mobileIcon} label={item.label} href={item.route} onSelect={handleMenuNavToggle} />
                     </div>
                  ))}
               </div>
            </div>
