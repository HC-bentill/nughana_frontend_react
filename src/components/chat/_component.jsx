...
function Chat() {
  ...
  return (
    <div className="w-full mt-4 rounded-lg">
      <div className="flex rounded-t-[10px] p-8  bg-white h-80">
        <div className="flex p-5 w-full overflow-y-auto rounded-[15px] bg-[#F5F5F5]">
          ...
          />
          <div>
            <h3 className="font-medium text-[17px]">NuGhana</h3>
            {messages.map((msg, i) => (
              <div
                key={i}
              ...
              >
                <p className="text-[16px] flex font-light">{msg.text}</p>
                <span className="block mt-2 text-xs text-right text-gray-500">
                  16:55
                </span>
              </div>
            ))}
